...
command_config_hx711s(uint32_t *args)
{
    ...
    sendf("debug_hx711s oid=%c arg[0]=%u "
        "arg[1]=%u arg[2]=%u arg[3]=%u",
        (int)args[0], (int)args[0], (int)args[1], 0, 0);
}
...
get_hx711s(struct hx711s* h)
{
    ...
    if (is_data_valid == 0)
    {
        ...
        sendf("result_hx711s oid=%c vd=%c it=%c "
            "tr=%hu nt=%u v0=%i v1=%i v2=%i v3=%i",
            (uint8_t)h->oid,
            (uint8_t)is_data_valid,
            (uint8_t)now_inter_ms,
            (uint16_t)h->times_read,
            (uint32_t)now_tick,
            (int32_t)outVals[0],
            (int32_t)outVals[1],
            (int32_t)outVals[2],
