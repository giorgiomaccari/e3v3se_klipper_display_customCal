...
class PRTouchCFG:
    def __init__(self, config):
        ...
        self.stored_profs = (
            self.stored_profs[1] if (len(self.stored_profs) == 2) else None
        )
        pass
...
class PRTouchZOffsetWrapper:
    def __init__(self, config):
        ...
        self.obj.gcode.register_command(
            ...
            self.cmd_PRTOUCH_PROBE_ZOFFSET,
            desc=self.cmd_PRTOUCH_PROBE_ZOFFSET_help,
        )
        self.obj.gcode.register_command(
            "NOZZLE_CLEAR",
            self.cmd_NOZZLE_CLEAR,
            desc=self.cmd_NOZZLE_CLEAR_help,
        )
        self.obj.gcode.register_command(
            "PRTOUCH_ACCURACY",
            self.cmd_PRTOUCH_ACCURACY,
            desc=self.cmd_PRTOUCH_ACCURACY_help,
        )
        pass
    ...
    def _move(self, pos, speed, wait=True):
        if (
            ...
            self.obj.gcode.run_script_from_command(
                "G1 F%d X%.3f Y%.3f Z%.3f"
                % (speed * 60, pos[0], pos[1], pos[2])
                if len(pos) >= 3
                else "G1 F%d X%.3f Y%.3f" % (speed * 60, pos[0], pos[1])
            )
            if wait:
    ...
    def _get_linear2(self, p1, p2, po, is_base_x):
        if (math.fabs(p1[0] - p2[0]) < 0.001 and is_base_x) or (
            ...
            return po
        a = (p2[2] - p1[2]) / (
            (p2[0] - p1[0]) if is_base_x else (p2[1] - p1[1])
        )
    ...
    def _check_trigger(
        self, arg_index, fit_vals, unfit_vals, min_hold, max_hold
    ):
        ...
        if (
            len(fit_vals) >= (self.cfg.pi_count / 2)
        ...
        for i in range(1, self.cfg.pi_count - 1):
            if (
                fit_vals_t[i] >= max_hold
                and fit_vals_t[i - 1] < (max_hold / 2)
                and fit_vals_t[i + 1] < (max_hold / 2)
            ):
        ...
        if self.val.out_index > 0:
            for i in range(self.val.out_index, self.cfg.pi_count):
                fit_vals_t[self.val.out_index] = fit_vals_t[
                    self.val.out_index
                ] * (self.obj.filter.lft_k1_oft / 2) + fit_vals_t[
                    self.val.out_index - 1
                ] * (
                    1 - (self.obj.filter.lft_k1_oft / 2)
                )
        vals_p = [x for x in fit_vals_t]
        ...
        if not (
            (fit_vals_t[-1] > max_val)
            and (fit_vals_t[-2] > max_val)
            and (fit_vals_t[-3] > max_val)
        ):
            return False
        ...
        for i in range(0, self.cfg.pi_count - 1):
            if (fit_vals_t[-1] - fit_vals_t[i]) / (
                (self.cfg.pi_count - i) * 1.0 / self.cfg.pi_count
            ) < 0.8:
                return False
        if (
            fit_vals[-1] < min_hold
    ...
    def _set_hot_temps(self, temp, fan_spd, wait=False, err=5):
        ...
        # self.obj.gcode.run_script_from_command('SET_PIN PIN=fan0 VALUE=%d' %
        # (fan_spd))
        # self.obj.gcode.run_script_from_command('SET_PIN PIN=fan1 VALUE=%d' %
        # (fan_spd))
        # self.obj.gcode.run_script_from_command('SET_PIN PIN=fan2 VALUE=%d' %
        # (fan_spd))
        if wait:
            while (
                self.ck_sys_sta()
                and abs(
                    self.obj.heater_hot.target_temp
                    - self.obj.heater_hot.smoothed_temp
                )
                > err
    ...
    def _set_bed_temps(self, temp, wait=False, err=5):
        ...
        if wait:
            while (
                self.ck_sys_sta()
                and abs(
                    self.obj.heater_bed.target_temp
                    - self.obj.heater_bed.smoothed_temp
                )
                > err
    ...
    def _probe_times(
        ...
        for i in range(max_times):
            ...
            self.pnt_msg(
                "***_probe_times must be reprobe= o_mm0=%.2f, o_mm1=%.2f"
                % (o_mm0, o_mm1)
            )
        return o_mm

    def clear_nozzle(
        self, hot_min_temp, hot_max_temp, bed_max_temp, min_hold, max_hold
    ):
        min_x, min_y = self.cfg.clr_noz_start_x, self.cfg.clr_noz_start_y
        max_x, max_y = (
            self.cfg.clr_noz_start_x + self.cfg.clr_noz_len_x,
            self.cfg.clr_noz_start_y + self.cfg.clr_noz_len_y,
        )
        ...
        self._set_hot_temps(
            temp=hot_min_temp + 40, fan_spd=0, wait=False, err=10
        )
        ...
        self._move(
            src_pos[:2] + [self.cfg.bed_max_err + 1], self.cfg.g29_xy_speed
        )
        ...
        self._move(
            end_pos[:2] + [end_pos[2] + self.cfg.pa_clr_down_mm],
            self.cfg.probe_speed,
        )
        self._set_hot_temps(temp=hot_min_temp, fan_spd=255, wait=True, err=5)
        self._move(
            [end_pos[0], end_pos[1] + 10, end_pos[2] + 10],
            self.cfg.probe_speed,
        )
        self._set_hot_temps(temp=hot_min_temp, fan_spd=0, wait=False)
    ...
    def _cal_min_z(self, start_z, hx711_vals):
        ...
        if dirzctl_params is None or len(dirzctl_params) != 2:
            raise self.obj.printer.command_error(
                """{"code":"key502", "msg":"probe_by_step: \
Can not recv stepper-z status."}"""
            )
        if (
            len(hx711_vals) < self.cfg.pi_count
            or len(hx711_params) < self.cfg.pi_count
        ):
            up_all_cnt = (
                dirzctl_params[0]["step"] - dirzctl_params[1]["step"] + 1
            )
        ...
        dirzctl_params[0]["tick"] = (
            dirzctl_params[0]["tick"] - dirzctl_start_tick
        ) / self.obj.dirzctl.mcu_freq
        dirzctl_params[1]["tick"] = (
            (
                4294967296
                if dirzctl_params[1]["tick"] < dirzctl_start_tick
                else 0
            )
            + dirzctl_params[1]["tick"]
            - dirzctl_start_tick
        ...
        dirzctl_params[1]["z"] = start_z - (
            dirzctl_params[0]["step"] - dirzctl_params[1]["step"] + 1
        ) * (
            self.obj.dirzctl.steppers[0].get_step_dist()
            * self.obj.dirzctl.step_base
        )
        tick_p = (
            (
                4294967296
                if hx711_params[self.val.out_index]["nt"] < hx711_start_tick
                else 0
            )
            + hx711_params[self.val.out_index]["nt"]
            - hx711_start_tick
        ) / self.obj.hx711s.mcu_freq
        self.val.out_val_mm = self._get_linear2(
        ...
        up_min_cnt = int(
            (self.val.out_val_mm - dirzctl_params[1]["z"])
            / (
                self.obj.dirzctl.steppers[0].get_step_dist()
                * self.obj.dirzctl.step_base
            )
    ...
    def probe_by_step(
        self, rdy_pos, speed_mm, min_dis_mm, min_hold, max_hold, up_after=True
    ):
        ...
        self.obj.dirzctl.check_and_run(
            0, int(step_us), int(step_cnt), wait_finish=False, is_ck_con=True
        )
        ...
        self.pnt_msg(
            "PROBE_BY_STEP x=%.2f y=%.2f z=%.2f \
speed_mm=%.2f step_us=%d step_cnt=%d"
        ...
        while self.ck_sys_sta():
            ...
            unfit_vals, tmp_unfit_vals = self.obj.filter.cal_offset_by_vals(
                self.obj.hx711s.s_count,
                all_valss,
                self.obj.filter.lft_k1_oft,
                self.cfg.pi_count,
            )
            fit_vals, tmp_fit_vals = self.obj.filter.cal_filter_by_vals(
                self.obj.hx711s.s_count,
                all_valss,
                self.obj.filter.hft_hz,
                self.obj.filter.lft_k1,
                self.cfg.pi_count,
            )
            ...
            for i in range(self.obj.hx711s.s_count):
                ...
                hx711_vals, tmp_hx711_vs = self.obj.filter.cal_filter_by_vals(
                    ...
                    all_valss,
                    self.obj.filter.hft_hz,
                    self.obj.filter.lft_k1_cal,
                    self.cfg.pi_count,
                )
                self.pnt_array(
                    "WAIT_AND_CAL_CH=%d, ARY=" % (i), tmp_hx711_vs[i]
                )
    ...
    def cmd_NOZZLE_CLEAR(self, gcmd):
        ...
        self.clear_nozzle(
            hot_min_temp, hot_max_temp, bed_max_temp, min_hold, max_hold
        )
    ...
    def cmd_PRTOUCH_PROBE_ZOFFSET(self, gcmd):
        ...
        if gcmd.get_int("CLEAR_NOZZLE", 0) == 1:
            self.clear_nozzle(
                self.cfg.hot_min_temp,
                self.cfg.hot_max_temp,
                self.cfg.bed_max_temp,
                self.cfg.min_hold,
                self.cfg.max_hold,
            )

        x = self.cfg.sensor_x + random.uniform(
            -self.cfg.random_offset, self.cfg.random_offset
        )
        y = self.cfg.sensor_y + random.uniform(
            -self.cfg.random_offset, self.cfg.random_offset
        )
        ...
        self.pnt_msg(
            "Checking z-position of probe (%.2f, %.2f)" % (probe_x, probe_y)
        )
        self._move(
            [probe_x, probe_y, self.cfg.bed_max_err + 1.0],
    ...
    def cmd_PRTOUCH_ACCURACY(self, gcmd):
        ...
        for i in range(len(positions)):
            deviation_sum += pow(positions[i] - avg_value, 2.0)
