...
class PRTouchCFG:
    def __init__(self, config):
        self.base_count = config.getint(
            'base_count', default=40, minval=10, maxval=100)
        self.pi_count = config.getint(
            'pi_count', default=32, minval=16, maxval=128)
        self.min_hold = config.getint(
            'min_hold', default=3000, minval=100, maxval=50000)
        self.max_hold = config.getint(
            'max_hold', default=50000, minval=100, maxval=100000)
        self.sensor_x = config.getfloat('sensor_x', minval=0, maxval=300)
        self.sensor_y = config.getfloat('sensor_y', minval=0, maxval=300)
        self.random_offset = config.getfloat(
            'sensor_random_offset', default=5, minval=0, maxval=10)
        self.hot_min_temp = config.getfloat(
            's_hot_min_temp', default=140, minval=80, maxval=200)
        self.hot_max_temp = config.getfloat(
            's_hot_max_temp', default=200, minval=180, maxval=300)
        self.bed_max_temp = config.getfloat(
            's_bed_max_temp', default=60, minval=45, maxval=100)
        self.pa_clr_dis_mm = config.getint(
            'pa_clr_dis_mm', default=5, minval=2, maxval=100)
        self.pa_clr_down_mm = config.getfloat(
            'pa_clr_down_mm', default=-0.1, minval=-1, maxval=1)
        self.clr_noz_start_x = config.getfloat(
            'clr_noz_start_x', default=0, minval=0, maxval=1000)
        self.clr_noz_start_y = config.getfloat(
            'clr_noz_start_y', default=0, minval=0, maxval=1000)
        self.clr_noz_len_x = config.getfloat(
            'clr_noz_len_x', default=0, minval=0, maxval=1000)
        self.clr_noz_len_y = config.getfloat(
            'clr_noz_len_y', default=0, minval=self.pa_clr_dis_mm + 5, maxval=1000)
        self.bed_max_err = config.getint(
            'bed_max_err', default=4, minval=2, maxval=10)
        self.max_z = config.getsection('stepper_z').getfloat(
            'position_max', default=300, minval=100, maxval=500)
        self.g29_xy_speed = config.getfloat(
            'g29_xy_speed', default=150, minval=10, maxval=1000)
        self.g29_rdy_speed = config.getfloat(
            'g29_rdy_speed', default=2.5, minval=0.1, maxval=10)
        self.probe_speed = config.getfloat(
            'probe_speed', default=2.0, minval=0.1, maxval=10)
        self.show_msg = config.getboolean('show_msg', default=False)
        self.check_bed_mesh_max_err = config.getfloat(
            'check_bed_mesh_max_err', default=0.2, minval=0.01, maxval=1)
        self.wipe_retract_distance = config.getfloat(
            'wipe_retract_distance', default=0, minval=0, maxval=50)
        self.probe_name = config.get('probe_name', default='bltouch')

        self.stored_profs = config.get_prefix_sections('prtouch')
        self.stored_profs = self.stored_profs[1] if (
            len(self.stored_profs) == 2) else None
        pass
...
class PRTouchZOffsetWrapper:
    def __init__(self, config):
        ...
        self.obj.gcode.register_command(
            'PRTOUCH_PROBE_ZOFFSET', self.cmd_PRTOUCH_PROBE_ZOFFSET, desc=self.cmd_PRTOUCH_PROBE_ZOFFSET_help)
        self.obj.gcode.register_command(
            'NOZZLE_CLEAR', self.cmd_NOZZLE_CLEAR, desc=self.cmd_NOZZLE_CLEAR_help)
        self.obj.gcode.register_command(
            'PRTOUCH_ACCURACY', self.cmd_PRTOUCH_ACCURACY, desc=self.cmd_PRTOUCH_ACCURACY_help)
        pass
    ...
    def _handle_mcu_identify(self):
        ...
        self.val.home_xy = [(max_x - min_x) / 2 + min_x,
                            (max_y - min_y) / 2 + min_y]
        pass
    ...
    def _move(self, pos, speed, wait=True):
        if not self.obj.hx711s.is_shutdown and not self.obj.hx711s.is_timeout and not self.obj.dirzctl.is_shutdown and not self.obj.dirzctl.is_timeout:
            ...
            self.obj.gcode.run_script_from_command('G1 F%d X%.3f Y%.3f Z%.3f' % (
                speed * 60, pos[0], pos[1], pos[2]) if len(pos) >= 3 else 'G1 F%d X%.3f Y%.3f' % (speed * 60, pos[0], pos[1]))
            if wait:
    ...
    def _check_trigger(self, arg_index, fit_vals,
        ...
        if (self.val.out_index > 0):
            for i in range(self.val.out_index, self.cfg.pi_count):
                fit_vals_t[self.val.out_index] = fit_vals_t[self.val.out_index] * (
                    self.obj.filter.lft_k1_oft / 2) + fit_vals_t[self.val.out_index - 1] * (1 - (self.obj.filter.lft_k1_oft / 2))
        vals_p = [x for x in fit_vals_t]
    ...
    def _probe_times(self, max_times, rdy_pos, speed_mm,
        ...
        for i in range(max_times):
            o_index0, o_mm0, deal_sta = self.probe_by_step(
                rdy_pos[:2] + [rdy_pos_z], speed_mm, min_dis_mm, min_hold, max_hold, True)
            if not deal_sta and rdy_pos_z == rdy_pos[2]:
                rdy_pos_z += 2
                continue
            o_index1, o_mm1, deal_sta = self.probe_by_step(
                rdy_pos[:2] + [rdy_pos_z], speed_mm, min_dis_mm, min_hold, max_hold, True)
            o_mm = (o_mm0 + o_mm1) / 2
    ...
    def clear_nozzle(self, hot_min_temp, hot_max_temp,
        ...
        src_pos[2] = self._probe_times(
            3, [src_pos[0], src_pos[1], src_pos[2]], self.cfg.probe_speed, 10, 0.2, min_hold, max_hold)
        self._set_hot_temps(temp=hot_min_temp + 40,
                            fan_spd=0, wait=False, err=10)
        end_pos[2] = self._probe_times(
            3, [end_pos[0], end_pos[1], end_pos[2]], self.cfg.probe_speed, 10, 0.2, min_hold, max_hold)
        self._move(src_pos[:2] + [self.cfg.bed_max_err + 1],
        ...
        self._move(end_pos[:2] + [end_pos[2] +
                   self.cfg.pa_clr_down_mm], self.cfg.probe_speed)
        self._set_hot_temps(temp=hot_min_temp, fan_spd=255, wait=True, err=5)
        self._move([end_pos[0], end_pos[1] + 10, end_pos[2] + 10],
    ...
    def probe_z_offset(self, x, y):
        self._ck_g28ed()
        z_offset = self._probe_times(3, [x, y, self.cfg.bed_max_err + 1.], self.cfg.probe_speed,
                                     10, self.cfg.check_bed_mesh_max_err, self.cfg.min_hold, self.cfg.max_hold)
        return z_offset
    ...
    def _cal_min_z(self, start_z, hx711_vals):
        ...
        dirzctl_params[0]['tick'] = (
            dirzctl_params[0]['tick'] - dirzctl_start_tick) / self.obj.dirzctl.mcu_freq
        dirzctl_params[1]['tick'] = ((4294967296 if dirzctl_params[1]['tick'] < dirzctl_start_tick else 0) +
                                     dirzctl_params[1]['tick'] - dirzctl_start_tick) / self.obj.dirzctl.mcu_freq
        dirzctl_params[0]['z'] = start_z
        dirzctl_params[1]['z'] = start_z - (dirzctl_params[0]['step'] - dirzctl_params[1]['step'] + 1) * (
            self.obj.dirzctl.steppers[0].get_step_dist() * self.obj.dirzctl.step_base)
        tick_p = ((4294967296 if hx711_params[self.val.out_index]['nt'] < hx711_start_tick else 0) +
                  hx711_params[self.val.out_index]['nt'] - hx711_start_tick) / self.obj.hx711s.mcu_freq
        self.val.out_val_mm = self._get_linear2([dirzctl_params[0]['tick'], 0, dirzctl_params[0]['z']], [
                                                dirzctl_params[1]['tick'], 0, dirzctl_params[1]['z']], [tick_p, 0, 0], True)[2]
        self.pnt_msg('call_min_z, re_probe_cnt=%d, out_index=%d, out_val_mm=%.2f' % (
            self.val.re_probe_cnt, self.val.out_index, self.val.out_val_mm))
        up_min_cnt = int((self.val.out_val_mm - dirzctl_params[1]['z']) / (
            self.obj.dirzctl.steppers[0].get_step_dist() * self.obj.dirzctl.step_base))
        up_all_cnt = dirzctl_params[0]['step'] - dirzctl_params[1]['step'] + 1
        limt_up_cnt = int(
            10 / (self.obj.dirzctl.steppers[0].get_step_dist() * self.obj.dirzctl.step_base))
    ...
    def probe_by_step(self, rdy_pos, speed_mm, min_dis_mm,
        ...
        self.obj.hx711s.query_start(
            self.cfg.pi_count * 2, int(65535), del_dirty=True, show_msg=False, is_ck_con=True)
        self.obj.dirzctl.check_and_run(0, int(step_us), int(
            step_cnt), wait_finish=False, is_ck_con=True)
        self.obj.hx711s.delay_s(0.015)
        self.pnt_msg(
            '*********************************************************')
        self.pnt_msg('PROBE_BY_STEP x=%.2f y=%.2f z=%.2f speed_mm=%.2f step_us=%d step_cnt=%d' % (
        ...
        while self.ck_sys_sta():
            ...
            unfit_vals, tmp_unfit_vals = self.obj.filter.cal_offset_by_vals(
                self.obj.hx711s.s_count, all_valss, self.obj.filter.lft_k1_oft, self.cfg.pi_count)
            fit_vals, tmp_fit_vals = self.obj.filter.cal_filter_by_vals(
            ...
            for i in range(self.obj.hx711s.s_count):
                ...
                self.obj.hx711s.query_start(
                    self.cfg.pi_count * 2, int(0), del_dirty=False, show_msg=False)
                self.obj.hx711s.delay_s(0.015)
                for j in range(int(self.obj.hx711s.s_count)):
                    self.pnt_array('TRIGGER_USE_CH=%d, FIT_VALS=' %
                                   (j), tmp_fit_vals[j], 16)
                self.obj.hx711s.delay_s(0.2)
                all_valss = self.obj.hx711s.get_vals()
                self.pnt_array('WAIT_AND_CAL_CH=%d, ARY=' % (i), all_valss[i])
                hx711_vals, tmp_hx711_vs = self.obj.filter.cal_filter_by_vals(
                    self.obj.hx711s.s_count, all_valss, self.obj.filter.hft_hz, self.obj.filter.lft_k1_cal, self.cfg.pi_count)
                self.pnt_array('WAIT_AND_CAL_CH=%d, ARY=' %
                               (i), tmp_hx711_vs[i])
                up_min_cnt, up_all_cnt, deal_sta = self._cal_min_z(
                    rdy_pos[2], tmp_hx711_vs[i])
                if up_after:
                    self.obj.dirzctl.check_and_run(
                        1, int(step_us / 2), int(up_all_cnt))
    ...
    def cmd_NOZZLE_CLEAR(self, gcmd):
        ...
        self.clear_nozzle(hot_min_temp, hot_max_temp,
                          bed_max_temp, min_hold, max_hold)
    ...
    def cmd_PRTOUCH_PROBE_ZOFFSET(self, gcmd):
        ...
        x = self.cfg.sensor_x + \
            random.uniform(-self.cfg.random_offset, self.cfg.random_offset)
        y = self.cfg.sensor_y + \
            random.uniform(-self.cfg.random_offset, self.cfg.random_offset)
        probe_x_offset, probe_y_offset = self.obj.probe.get_offsets()[:2]
        probe_x = x - probe_x_offset
        probe_y = y - probe_y_offset
        self.pnt_msg("Checking z-position of probe (%.2f, %.2f)" %
                     (probe_x, probe_y))
        self._move([probe_x, probe_y, self.cfg.bed_max_err + 1.],
                   self.cfg.g29_xy_speed)
    ...
    def cmd_PRTOUCH_ACCURACY(self, gcmd):
        ...
        gcmd.respond_info("PRTOUCH_ACCURACY at X:%.3f Y:%.3f"
                          ...
                          % (self.cfg.sensor_x, self.cfg.sensor_y,
                             sample_count, speed))
        sensor_pos = [self.cfg.sensor_x,
                      self.cfg.sensor_y, self.cfg.bed_max_err]
        ...
        while len(positions) < sample_count:
            # Probe position
            _index1, pos, _sta = self.probe_by_step(
                sensor_pos, speed, 10, self.cfg.min_hold, self.cfg.max_hold, True)
