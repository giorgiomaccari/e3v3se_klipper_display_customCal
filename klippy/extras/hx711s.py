...
class HX711S:
    def __init__(self, config):
        ...
        for i in range(self.s_count):
            self.s_clk_pin.append(config.get(
                'sensor%d_clk_pin' % i, None if i == 0 else self.s_clk_pin[i - 1]))
            self.s_sdo_pin.append(config.get(
                'sensor%d_sdo_pin' % i, None if i == 0 else self.s_sdo_pin[i - 1]))
    ...
    def _build_config(self):
        ...
        for i in range(self.s_count):
            ...
            self.mcu.add_config_cmd("add_hx711s oid=%d index=%d clk_pin=%s sdo_pin=%s" % (
        ...
        for i in range(3):
            ...
            if len(vals[0]) < cnt:
                raise self.printer.command_error(
                    """{"code":"key503", "msg":"z-Touch::read_base: Can not read z-Touch data."}""")
            ...
            for j in range(self.s_count):
                ...
                self.printer.lookup_object('prtouch').pnt_msg(
                    'READ_BASE ch=%d min=%.2f avg=%.2f max=%.2f' % (j, rvs[j][-3], avgs[j], rvs[j][-1]))
            if reset_zero:
                self.base_avgs = avgs
