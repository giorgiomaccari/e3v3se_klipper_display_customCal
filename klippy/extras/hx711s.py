...
class HX711S:
    def __init__(self, config):
        ...
        for i in range(self.s_count):
            self.s_clk_pin.append(
                config.get(
                    "sensor%d_clk_pin" % i,
                    None if i == 0 else self.s_clk_pin[i - 1],
                )
            )
            self.s_sdo_pin.append(
                config.get(
                    "sensor%d_sdo_pin" % i,
                    None if i == 0 else self.s_sdo_pin[i - 1],
                )
            )
        self.mcu = mcu.get_printer_mcu(
            self.printer, config.get("use_mcu", "mcu")
        )
    ...
    def _build_config(self):
        ...
        for i in range(self.s_count):
            ...
            self.mcu.add_config_cmd(
                ...
                % (self.oid, i, clk_pin_params["pin"], sdo_pin_params["pin"])
            )
        # self.query_cmd = self.mcu.lookup_command(
        # "query_hx711s oid=%c times_read=%hu is_ck_con=%c",
        # cq=None)
        self.query_cmd = self.mcu.lookup_command(
            "query_hx711s oid=%c times_read=%hu", cq=None
        )
    ...
    def _handle_result_hx711s(self, params):
        while self.need_wait:
            self.delay_s(0.001)
        self.start_tick = (
            self.start_tick if len(self.all_params) != 0 else params["nt"]
        )
        if (
            self.del_dirty
            and (params["vd"] != 0 or params["it"] > 20)
            and self.index_dirty == 0
        ):
            self.index_dirty = 1
            return
    ...
    def read_base(self, cnt, max_hold, reset_zero=True):
        ...
        for i in range(3):
            ...
            t_last = time.time()
            while (
                not (self.is_shutdown or self.is_timeout)
                and len(self.get_vals()[0]) < cnt
                and (time.time() - t_last) < cnt * 0.010 * 15
            ):
                self.delay_s(0.010)
                pass
            vals = self.get_vals()
            if len(vals[0]) < cnt:
                raise self.printer.command_error(
                    """{"code":"key503", "msg":"z-Touch::read_base: \
Can not read z-Touch data."}"""
                )
            ...
            for j in range(self.s_count):
                ...
                self.printer.lookup_object("prtouch").pnt_msg(
                    "READ_BASE ch=%d min=%.2f avg=%.2f max=%.2f"
                    % (j, rvs[j][-3], avgs[j], rvs[j][-1])
                )
            if reset_zero:
                self.base_avgs = avgs
